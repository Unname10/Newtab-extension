...
window.onload = async function () {
    //Variables
    var storageData = await chrome.storage.local.get();
    ...
    function bg(trans) {
        if (trans)
            autoChange = Module.transitionBg(
                bgElement,
                bgElement2,
                setting.imgSlideTime,
                setting.numberOfPicture
            );
    ...
    if (setting.imgSlide) {
        // Kiểm tra xem có bật chuyển ảnh hay không
    ...
    Module.addContent(
        ...
        recent,
        'Bạn chưa truy cập trang nào gần đây!'
    );
    Module.addContent(
        favoriteContentElement,
        favorite,
        'Bạn chưa truy cập trang nào gần đây!'
    );
    ...
    recentElement.addEventListener('mouseenter', () =>
        Module.toggle(recentContentElement, true)
    );
    recentElement.addEventListener('mouseleave', () =>
        Module.toggle(recentContentElement, false)
    );
    favoriteElement.addEventListener('mouseenter', () =>
        Module.toggle(favoriteContentElement, true)
    );
    favoriteElement.addEventListener('mouseleave', () =>
        Module.toggle(favoriteContentElement, false)
    );
    bookmarkElement.addEventListener('mouseenter', () =>
        Module.toggle(bookmarkContentElement, true)
    );
    bookmarkElement.addEventListener('mouseleave', () =>
        Module.toggle(bookmarkContentElement, false)
    );
    settingElement.addEventListener('mouseenter', () =>
        Module.toggle(settingContentElement, true)
    );
    settingElement.addEventListener('mouseleave', () =>
        Module.toggle(settingContentElement, false)
    );
    ...
    trashIcon.forEach((element) => {
        element.onclick = () => {
            ...
            chrome.storage.local.set({ bookmark: bookmark });
            bookmarkInit();
        };
    });
    ...
    effectRuntimeChange.onchange = () => {
        ...
        $(':root').style.setProperty(
            '--animation-duration',
            `${effectRuntimeChange.value}s`
        );
        // Cập nhật giá trị vào storage
        setting.effectRuntime = effectRuntimeChange.value;
        chrome.storage.local.set({ effectRuntime: setting });
    };
